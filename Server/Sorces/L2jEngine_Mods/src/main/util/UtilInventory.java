...
public class UtilInventory
{
	...
	public static void giveItems(PlayerHolder ph, int itemId, int itemCount, int enchantLevel)
	{
		...
		if (itemId == 57)
		{
			player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_HAVE_EARNED_S1_ADENA).addNumber(itemCount));
		}
		else
		{
			if (itemCount > 1)
			{
				player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_HAVE_EARNED_S2_S1S).addItemName(itemId).addNumber(itemCount));
			}
			else
			{
	...
	public static boolean takeItems(PlayerHolder ph, int itemId, int itemCount)
	{
		...
		ph.getInstance().sendPacket(new ItemList(ph.getInstance(), false));
