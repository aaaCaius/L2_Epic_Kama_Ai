...
public class SystemVip extends AbstractMod
{
	...
	public boolean onUseItem(PlayerHolder ph, ItemHolder item)
	{
		if (ConfigData.VIP_ITEM_ID > 0)
		{
			...
			if (item.getInstance().getItemId() == ConfigData.VIP_ITEM_ID)
			{
				// Create calendar
				GregorianCalendar time = new GregorianCalendar();
				time.add(Calendar.DAY_OF_YEAR, ConfigData.VIP_ITEM_DAYS);
				// save values in DB
				setValueDB(ph, "vip", time.getTimeInMillis() + "");
				// saved state in memory
				ph.setVip(true, time.getTimeInMillis());
				
				addVip(ph, time.getTimeInMillis());
				
				// Informed player
				UtilMessage.sendCreatureMsg(ph, Say2.TELL, "[System]", "Dear " + ph.getName() + " your are now Vip");
				
				informeExpireVip(ph);
				
				ph.getInstance().destroyItemByItemId("vip item", item.getInstance().getItemId(), 1, null, true);
			}
		}
	...
	private void informeExpireVip(PlayerHolder ph)
	{
		...
		// hb.append(Html.image(L2UI.SquareGray, 264, 1));
		...
		hb.append("<tr>");
		hb.append("<td fixwidth=100 align=center><font color=0FB614>EXP:</font></td>");
		...
		hb.append("<table fixwidth=264>");
		hb.append("<tr>");
		hb.append("<td fixwidth=100 align=center><font color=0FB614>SP:</font></td>");
		...
		hb.append("<table fixwidth=264 border=0 cellspacing=0 cellpadding=0>");
		hb.append("<tr>");
		hb.append("<td fixwidth=100 align=center><font color=0FB614>Normal:</font></td>");
		...
		hb.append("<table fixwidth=264 border=0 cellspacing=0 cellpadding=0>");
		hb.append("<tr>");
		hb.append("<td fixwidth=100 align=center><font color=0FB614>Spoil:</font></td>");
		...
		hb.append("<table fixwidth=264 border=0 cellspacing=0 cellpadding=0>");
		hb.append("<tr>");
		hb.append("<td fixwidth=100 align=center><font color=0FB614>Seed:</font></td>");
