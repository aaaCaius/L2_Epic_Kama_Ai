...
public class NpcBufferScheme extends AbstractMod
{
	...
	private static final int MAX_SCHEME_DANCES = 14;
	private static final int MAX_SCHEME_CUBIC = 1;
	...
	{
		if (!Util.areObjectType(L2NpcInstance.class, npc))
		{
			return;
		}
		...
		switch (bypass)
		{
			...
			case "buffpet":
			{
				if (checkTimeOut(ph))
				{
					...
					if (TIME_OUT)
					{
						addTimeout(ph, SetupGauge.GREEN, TIME_OUT_TIME / 2, 600);
					}
				}
				rebuildMainHtml(ph);
				return;
			...
			case "create":
			{
				...
				{
					// TODO missing INCORRECT_NAME_TRY_AGAIN
					ph.getInstance().sendPacket(new SystemMessage(204));
					showText(ph, "Info", "Please, enter the scheme name!", true, "Return", "main");
					return;
				}
				
				// get the list of schemes
				String allSchemes = getValueDB(ph.getObjectId(), SCHEME_NAME).getString();
			...
			case "delete":
			{
				...
				String schemes = getValueDB(ph.getObjectId(), SCHEME_NAME).getString();
			...
			case "heal":
			{
				if (checkTimeOut(ph))
				{
					...
					if (TIME_OUT)
					{
						addTimeout(ph, SetupGauge.BLUE, TIME_OUT_TIME / 2, 600);
					}
				}
				return;
			}
			case "removeBuffs":
			{
				if (checkTimeOut(ph))
				{
					...
					else
					{
						...
						if (ph.getInstance().getCubics() != null)
						{
							for (L2CubicInstance cubic : ph.getInstance().getCubics().values())
							{
								...
								ph.getInstance().delCubic(cubic.getId());
							}
						}
					}
					...
					if (TIME_OUT)
					{
						addTimeout(ph, SetupGauge.RED, TIME_OUT_TIME / 2, 600);
					}
				}
			...
			case "cast":
			{
				if (checkTimeOut(ph))
				{
					...
					boolean getPetbuff = isPetBuff(ph);
					...
					for (BuffHolder bh : buffs)
					{
						if (!getPetbuff)
						{
							if (IsCubic(bh.getId()))
							{
								// removing all cubics before adding the cubic from the schema.
								// only one cubic is allowed by design
								for (L2CubicInstance cubic : ph.getInstance().getCubics().values())
								{
									cubic.stopAction();
									cubic.cancelDisappear();
								...
								SetCubic(ph, bh.getId(), bh.getLevel(), SkillTable.getInstance().getInfo(bh.getId(), bh.getLevel()));
							}
							else
							{
								SkillTable.getInstance().getInfo(bh.getId(), bh.getLevel()).getEffects(ph.getInstance(), ph.getInstance());
							}
						...
						else
						{
							if (ph.getInstance().getPet() != null)
							{
								SkillTable.getInstance().getInfo(bh.getId(), bh.getLevel()).getEffects(ph.getInstance().getPet(), ph.getInstance().getPet());
							}
							else
							{
								showText(ph, "Info", "You can't use the Pet's options.<br>Summon your pet first!", false, "Return", "main");
					...
					if (TIME_OUT)
					{
						addTimeout(ph, SetupGauge.CYAN, TIME_OUT_TIME, 600);
					}
				}
				rebuildMainHtml(ph);
				return;
			...
			case "giveBuffs":
			{
				...
				if (checkTimeOut(ph))
				{
					...
					if (getPetbuff)
					{
						...
						if (!IsCubic(Integer.parseInt(eventParam1)))
						{
							skill.getEffects(ph.getInstance().getPet(), ph.getInstance().getPet());
						}
						else
						{
							showText(ph, "Info", "Sorry but I cannot summon a cubic for a pet", true, "Return", "main");
						}
					...
					else
					{
						...
						if (IsCubic(Integer.parseInt(eventParam1)))
						{
							SetCubic(ph, Integer.parseInt(eventParam1), Integer.parseInt(eventParam2), skill);
						}
						else
						{
					...
					if (TIME_OUT)
					{
						addTimeout(ph, SetupGauge.CYAN, TIME_OUT_TIME / 10, 600);
					}
				}
				return;
	...
	{
		String blockUntilTime = getValueDB(ph.getObjectId(), BLOCK_UNTIL_TIME).getString();
	...
	private void addTimeout(PlayerHolder ph, int gauge, int amount, int offset)
	{
	...
	private static void heal(PlayerHolder ph, boolean isPet)
	{
		...
		else if (target != null)
		{
			...
			if (target instanceof L2PetInstance)
			{
				...
				ph.getInstance().sendPacket(new SetSummonRemainTime(pet.getPetData().getPetMaxFeed(), pet.getCurrentFed()));
			}
		}
	}
	...
	private void rebuildMainHtml(PlayerHolder player)
	{
		...
		hb.append("<td height=32 align=center><button value=Dances action=\"bypass -h Engine NpcBufferScheme redirect_view_dance\" width=75 height=21 back=", L2UI_CH3.Btn1_normalOn, " fore=", L2UI_CH3.Btn1_normal, "></td>");
		hb.append("</tr>");
		
		hb.append("<tr>");
		hb.append("<td height=32 align=center><button value=Chants action=\"bypass -h Engine NpcBufferScheme redirect_view_chant\" width=75 height=21 back=", L2UI_CH3.Btn1_normalOn, " fore=", L2UI_CH3.Btn1_normal, "></td>");
		...
		hb.append("<tr>");
		hb.append("<td><button value=\"", bottonA, "\" action=\"bypass Engine NpcBufferScheme castBuffSet 0 0 0\" width=75 height=21 back=", L2UI_CH3.Btn1_normalOn, " fore=", L2UI_CH3.Btn1_normal, "></td>");
		hb.append("<td><button value=\"", bottonB, "\" action=\"bypass Engine NpcBufferScheme heal\" width=75 height=21 back=", L2UI_CH3.Btn1_normalOn, " fore=", L2UI_CH3.Btn1_normal, "></td>");
		hb.append("<td><button value=\"", bottonC, "\" action=\"bypass Engine NpcBufferScheme removeBuffs 0 0 0\" width=75 height=21 back=", L2UI_CH3.Btn1_normalOn, " fore=", L2UI_CH3.Btn1_normal, "></td>");
		hb.append("</tr>");
		hb.append("</table>");
	...
	{
		String schemeNames = getValueDB(ph.getObjectId(), SCHEME_NAME).getString();
	...
	private static void buildHtml(PlayerHolder ph, BuffType buffType, int page)
	{
		...
		else
		{
			...
			for (BuffHolder bh : buffs)
			{
				...
				hb.append("<td height=32 fixwidth=232 align=center><a action=\"bypass Engine NpcBufferScheme giveBuffs ", bh.getId(), " ", bh.getLevel(), " ", buffType.name(), " ", page, "\">", name, "</a></td>");
				hb.append("<td height=32 fixwidth=32>", getSkillIconHtml(bh.getId(), bh.getLevel()), "</td>");
				hb.append("</tr>");
	...
	private String viewAllSchemeBuffsGetBuffCount(PlayerHolder ph, String scheme)
	{
		...
		return count + " " + B_Count + " " + D_S_Count + " " + C_Count;
	}
	...
	private void viewAllSchemeBuffs(PlayerHolder ph, String schemeName, String page, String action)
	{
		...
		int CUBIC_COUNT = Integer.parseInt(eventSplit[3]);
		List<BuffHolder> buffs = new ArrayList<>();
		
		if (action.equals("add"))
		{
			hb.append("You can add <font color=\"LEVEL\">", MAX_SCHEME_BUFFS - BUFF_COUNT, "</font> Buffs and <font color=\"LEVEL\">", MAX_SCHEME_DANCES - DANCE_SONG, "</font> Dances and <font color=\"LEVEL\">", MAX_SCHEME_CUBIC - CUBIC_COUNT, "</font> Cubics more !");
			...
			for (BuffHolder bh : SchemeBufferPredefinedData.getAllGeneralBuffs())
			{
				...
				{
					if ((bh.getType() != BuffType.DANCE) && (bh.getType() != BuffType.SONG) && (bh.getType() != BuffType.CUBIC))
					{
						continue;
					}
				}
				
				if (CUBIC_COUNT >= MAX_SCHEME_CUBIC)
				{
					if (bh.getType() == BuffType.CUBIC)
					{
						continue;
	...
	private static void SetCubic(PlayerHolder Ph, int Id, int Level, L2Skill skill)
	{
		switch (Id)
		{
			case 10:
			{
				Ph.getInstance().addCubic(1, Level, skill.getPower(), 12, 10, 900000, false);
				break;
			}
			case 22:
			{
				Ph.getInstance().addCubic(2, Level, skill.getPower(), 15, 8, 900000, false);
				break;
			}
			case 33:
			{
				Ph.getInstance().addCubic(5, Level, skill.getPower(), 8, 30, 900000, false);
				break;
			}
			case 67:
			{
				Ph.getInstance().addCubic(3, Level, skill.getPower(), 10, 80, 900000, false);
				break;
			}
			case 278:
			{
				Ph.getInstance().addCubic(4, Level, skill.getPower(), 14, 55, 900000, false);
				break;
			}
			case 1279:
			{
				Ph.getInstance().addCubic(6, Level, skill.getPower(), 30, 12, 900000, false);
				break;
			}
			case 1280:
			{
				Ph.getInstance().addCubic(7, Level, skill.getPower(), 30, 30, 900000, false);
				break;
			}
			case 1281:
			{
				Ph.getInstance().addCubic(8, Level, skill.getPower(), 30, 12, 900000, false);
				break;
			}
			default:
			{
	...
	private static boolean IsCubic(int Id)
	{
		switch (Id)
		{
			case 10:
