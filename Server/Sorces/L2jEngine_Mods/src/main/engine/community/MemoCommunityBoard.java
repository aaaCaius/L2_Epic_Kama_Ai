...
public class MemoCommunityBoard extends AbstractMod
{
	...
	private static String getSellList(PlayerHolder ph, int page)
	{
		...
		hb.append("you will be charged ", Html.formatAdena(ConfigData.COMMISION_FOR_START_SELL), " the ", ItemTable.getInstance().getTemplate(ConfigData.COMMISION_FOR_START_SELL_ID).getName(), "<br1>");
	...
	private static void giveAuctionSold(L2PcInstance owner, AuctionItemHolder ih)
	{
		...
		// mensajes
		owner.sendPacket(new CreatureSay(owner.getObjectId(), Say2.TELL, "Auction", "You have sold the item " + item.getName()));
