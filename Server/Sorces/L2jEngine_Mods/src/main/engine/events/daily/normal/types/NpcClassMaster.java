...
public class NpcClassMaster extends AbstractEvent
{
	...
	{
		if (!ConfigData.CLASSMASTER_MESSAGE_START.isEmpty())
		{
			UtilMessage.sendAnnounceMsg(ConfigData.CLASSMASTER_MESSAGE_START, ph);
		}
	}
	...
	public void onEvent(PlayerHolder ph, CharacterHolder npc, String event)
	{
		...
		if (getState() == EngineStateType.END)
		{
			ph.getInstance().sendMessage("Event Class Master not in progress!");
			ph.getInstance().sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
