...
public class LoginController
{
	...
	public boolean isLoginPossible(final L2LoginClient client, final int serverId)
	{
		...
		if (gsi != null && gsi.isAuthed())
		{
			final boolean loginOk = gsi.getCurrentPlayerCount() < gsi.getMaxPlayers() && gsi.getStatus() != ServerStatus.STATUS_GM_ONLY || client.isGM();
	...
	public synchronized boolean loginValid(String user, String password, L2LoginClient client)
	{
		...
		if (!ok)
		{
			L2Log.add("'" + user + "' " + address.getHostAddress(), "logins_ip_fails");
		...
		else
		{
			...
			// General log
			L2Log.add("'" + user + "' " + address.getHostAddress(), "logins_ip");
			// Individual log
			L2Log.add("'" + user + "' " + address.getHostAddress(), "accounts", user);
		}
