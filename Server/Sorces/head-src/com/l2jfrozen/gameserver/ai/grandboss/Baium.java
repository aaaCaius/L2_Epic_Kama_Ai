...
public class Baium extends Quest implements Runnable
{
	...
	private final static int ANGEL_LOCATIONS[][] =
	{
		{
			114239,
			17168,
	...
	public String onAdvEvent(final String event, final L2NpcInstance npc, final L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("baium_wakeup") && npc != null)
		{
			if (npc.getNpcId() == LIVE_BAIUM)
			{
				...
				for (int[] location : ANGEL_LOCATIONS)
				{
					final L2MonsterInstance angel = (L2MonsterInstance) addSpawn(ARCHANGEL, location[0], location[1], location[2], location[3], false, 0);
