...
{
	private static final Logger LOGGER = Logger.getLogger(CharTemplateTable.class);
	private static final String SELECT_CHARACTER_TEMPLATE = "SELECT * FROM char_templates LEFT JOIN lvlupgain ON char_templates.ClassId = lvlupgain.classid";
	...
	private CharTemplateTable()
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(SELECT_CHARACTER_TEMPLATE);
			ResultSet rset = statement.executeQuery())
		{
			while (rset.next())
			{
				StatsSet set = new StatsSet();
				set.set("classId", rset.getInt("ClassId"));
