	...
	public void processHtml(L2GameClient paramGameClient)
	{
		...
		if (filePath != null)
		{
			if (localPlayer != null)
			{
				if (localPlayer.isGM())
				{
					localPlayer.sendMessage("HTML: " + filePath);
				}
				
				if(filePath.contains("teleporter/") && localPlayer.getLevel() <= Config.ALT_TELEPORT_FREE_UNTIL_LEVEL)
				{
					html = html.replaceAll(" - [0-9]+ Adena</a>", "</a>");
				}
			}
		}
	}
