	...
	protected final void writeImpl()
	{
		...
		switch (page)
		{
			case 1:
				...
				writeC(SevenSigns.getInstance().getPlayerCabal(activeChar));
				writeC(SevenSigns.getInstance().getPlayerSeal(activeChar));
				
				writeD(SevenSigns.getInstance().getPlayerStoneContrib(activeChar)); // Seal Stones Turned-In
				writeD(SevenSigns.getInstance().getPlayerAdenaCollect(activeChar)); // Ancient Adena to Collect
