...
public final class L2GameClient extends MMOClient<MMOConnection<L2GameClient>> implements Runnable
{
	...
	public void sendPacket(final L2GameServerPacket gsp)
	{
		...
		if (getConnection() != null)
		{
			...
			{
				L2Log.add("[ServerPacket] SendingGameServerPacket, Client: " + toString() + " Packet:" + gsp.getType(), "GameServerPacketsLog");
			}
	...
	public static void deleteCharByObjId(int playerObjId)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM character_recipes WHERE char_id=?"))
			{
				statement.setInt(1, playerObjId);
				statement.executeUpdate();
			}
			
			try (PreparedStatement statement = con.prepareStatement("DELETE FROM character_seven_signs WHERE char_obj_id=?"))
			{
				statement.setInt(1, playerObjId);
				statement.executeUpdate();
			}
	...
	public void onForcedDisconnection(final boolean critical)
	{
		...
		if (critical && Config.ENABLE_ALL_EXCEPTIONS)
		{
			...
			L2Log.add(text, "Chars_disconnection_logs");
		}
				...
				if (player != null) // this should only happen on connection loss
				{
					if (Config.ENABLE_OLYMPIAD_DISCONNECTION_DEBUG)
					{
						if (player.isInOlympiadMode() || player.inObserverMode())
						{
							...
							{
								String text = "Player " + player.getName() + ", Class:" + player.getClassId() + ", Level:" + player.getLevel() + ", Mode: Olympiad, Loc: " + player.getX() + " Y:" + player.getY() + " Z:" + player.getZ() + ", Critical?: " + forcedToClose;
								L2Log.add(text, "olympiad", "olympiad_crash_debug");
							}
							else if (player.inObserverMode())
							{
								String text = "Player " + player.getName() + ", Class:" + player.getClassId() + ", Level:" + player.getLevel() + ", Mode: Observer, Loc: " + player.getX() + " Y:" + player.getY() + " Z:" + player.getZ() + ", Critical?: " + forcedToClose;
								L2Log.add(text, "olympiad", "olympiad_crash_debug");
							}
							else
							{
								String text = "Player " + player.getName() + ", Class:" + player.getClassId() + ", Level:" + player.getLevel() + ", Mode: Default, Loc: " + player.getX() + " Y:" + player.getY() + " Z:" + player.getZ() + ", Critical?: " + forcedToClose;
								L2Log.add(text, "olympiad", "olympiad_crash_debug");
							}
						}
					}
					...
					if (Config.ENABLE_OLYMPIAD_DISCONNECTION_DEBUG)
					{
							...
							|| player.inObserverMode())
						{
							if (player.isInOlympiadMode())
							{
								...
								L2Log.add(text, "Olympiad_crash_debug");
							}
							else if (player.inObserverMode())
							{
	...
	public String getIPAddress()
	{
		try
		{
			MMOConnection<L2GameClient> conection = getConnection();
			InetAddress ipAddress = conection.getInetAddress();
			return ipAddress.getHostAddress();
		}
		catch (Exception e)
		{
			LOGGER.warn("Something went wrong while getting IP ADDRESS from CLIENT", e);
			return "0.0.0.0";
		}
	}
}
