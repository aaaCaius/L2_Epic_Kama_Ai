...
public final class UseItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (item.isEquipable())
		{
			...
			if (Config.MAX_ENCHANT_LEVEL > 0 && !activeChar.isGM() && item.getEnchantLevel() > Config.MAX_ENCHANT_LEVEL)
			{
			...
			if (Config.MAX_ENCHANT_LEVEL > 0 && !activeChar.isGM())
			{
				if (item.getEnchantLevel() > Config.MAX_ENCHANT_LEVEL)
				{
		...
		EngineModsManager.onUseItem(activeChar, item);
	}
