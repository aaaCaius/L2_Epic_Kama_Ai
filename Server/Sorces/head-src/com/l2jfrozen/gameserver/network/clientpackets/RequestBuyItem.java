	...
	protected void runImpl()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (price == 0 && !player.isGM())
			{
				...
				String message = "Warning!! Character " + player.getName() + " of account " + player.getAccountName() + " tried buy item for 0 adena.";
				LOGGER.warn(message);
				Util.handleIllegalPlayerAction(player, message, Config.DEFAULT_PUNISH);
				return;
			}
