...
public final class RequestRefine extends L2GameClientPacket
{
	...
	boolean tryAugmentItem(final L2PcInstance player, final L2ItemInstance targetItem, final L2ItemInstance refinerItem, final L2ItemInstance gemstoneItem)
	{
		...
		if (!LifeStoneTable.getAllLifeStonesIds().contains(lifeStoneId))
		{
			return false;
		}
		...
		if (!player.destroyItemByItemId("Request Refine", lifeStoneId, 1, player, true))
		{
			return false;
		}
		
		// consume the gemstones
		player.destroyItemByItemId("Request Refine", gemstoneItemId, modifyGemstoneCount, player, true);
