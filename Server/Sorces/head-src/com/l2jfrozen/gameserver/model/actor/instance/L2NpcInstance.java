...
public class L2NpcInstance extends L2Character
{
	...
	public void showQuestWindow(final L2PcInstance player)
	{
		...
		if (awaits != null)
		{
			for (final QuestState x : awaits)
			{
				...
				{
					if (x.getQuest().getQuestId() > 0 && x.getQuest().getQuestId() < 1000)
					{
		...
		for (final Quest x : starts)
		{
			...
			{
				if (x.getQuestId() > 0 && x.getQuestId() < 1000)
				{
					options.add(x);
				}
			}
	...
	public void makeSupportMagic(L2PcInstance player)
	{
		...
		int lowestLevel = NewbieGuideBuffTable.getInstance().getLowerLevel();
		int higestLevel = NewbieGuideBuffTable.getInstance().getHighestLevel();
		
		// Select the player
		setTarget(player);
		
		// If the player is too high level, display a message and return
		if (player.getLevel() > higestLevel || !player.isNewbie())
		{
		...
		for (NewbieGuideBuffHolder buff : NewbieGuideBuffTable.getInstance().getNewbieHelperBuffsForPlayer(player))
		{
	...
	public void showChatWindow(final L2PcInstance player, final int val)
	{
		...
		switch (npcId)
		{
			...
			case 31113: // Merchant of Mammon
				if (Config.ALT_REQUIRE_WIN_7S)
				{
					System.out.println("COMPWINNER: [" + compWinner + "]");
					System.out.println("PLAYER CABAL: [" + playerCabal + "]");
					System.out.println("SEAL AVARICE: [" + sealAvariceOwner + "]");
