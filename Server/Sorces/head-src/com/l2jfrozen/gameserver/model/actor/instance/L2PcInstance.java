...
import com.l2jfrozen.gameserver.model.holder.ItemHolder;
...
import com.l2jfrozen.gameserver.templates.AccessLevel;
...
import com.l2jfrozen.gameserver.util.DualBox;
...
public class L2PcInstance extends L2PlayableInstance
{
	...
	private static final String INSERT_CHARACTER_KILLS = "INSERT INTO character_kills (char_obj_id,char_killer_obj_id,kills) VALUES (?,?,?)";
	...
	private static final String SELECT_CHARACTER_RECIPES = "SELECT id FROM character_recipes WHERE char_id=?";
	private static final String INSERT_CHARACTER_RECIPE = "INSERT INTO character_recipes (char_id, id) VALUES(?,?)";
	private static final String DELETE_CHARARACTER_RECIPE = "DELETE FROM character_recipes WHERE char_id=? AND id=?";
	...
	private final Map<Integer, Recipe> dwarvenRecipeBook = new HashMap<>();
	private final Map<Integer, Recipe> commonRecipeBook = new HashMap<>();
	...
	/**
	 * Adventurers' coupon 0 = No coupon 1 = coupon for No Grade 2 = coupon for D Grade 3 = coupon for No & D Grade
	 **/
	...
	private ItemHolder teleportItem = new ItemHolder(0, 0);
	...
	public int getLevel()
	{
		...
		if (level == -1)
		{
			L2PcInstance localChar = restore(getObjectId());
	 ...
	 */
	public Recipe[] getCommonRecipeBook()
	{
		return commonRecipeBook.values().toArray(new Recipe[commonRecipeBook.values().size()]);
	}
	 ...
	 */
	public Recipe[] getDwarvenRecipeBook()
	{
		return dwarvenRecipeBook.values().toArray(new Recipe[dwarvenRecipeBook.values().size()]);
	}
	...
	public void unregisterRecipeList(int recipeId)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement deleteRecipe = con.prepareStatement(DELETE_CHARARACTER_RECIPE))
		{
		...
		{
			LOGGER.error("L2PcInstance.unregisterRecipeList : Could not delete recipe book data", e);
		}
	 ...
	 * @param item   the item
	 */
	public void doAutoLoot(final L2Attackable target, final RewardHolder item)
	{
		if (isInParty())
		{
			getParty().distributeItem(this, item, false, target);
		}
		else if (item.getRewardId() == 57)
		{
			addAdena("AutoLoot", item.getRewardCount(), target, true);
		}
		else
		{
			addItem("AutoLoot", item.getRewardId(), item.getRewardCount(), target, true);
		}
	}
	...
	public boolean doDie(final L2Character killer)
	{
		...
		if (killer != null)
		{
			...
			if (pk != null)
			{
				if (Config.PVP_AND_PK_INFO_ENABLED)
				{
					calculatePvPAndPKInfo(pk);
				}
			}
	...
	public void onKillUpdatePvPKarma(final L2Character target)
	{
		...
		if (checkIfPvP(target) && targetPlayer.getPvpFlag() != 0)
		{
			...
			{
				L2World.getInstance().getAllPlayers().forEach(player -> player.sendPacket(new CreatureSay(0, Say2.TELL, "[PvP]", getName() + " has defeated " + target.getName())));
			}
		}
		else
		{
			...
			{
				if (Config.KILL_PK_INCREASE_PVP)
				{
					increasePvpKills();
				}
			...
			else if (targetPlayer.getPvpFlag() == 0) // Target player doesn't have karma
			{
				...
				{
					L2World.getInstance().getAllPlayers().forEach(player -> player.sendPacket(new CreatureSay(0, Say2.PARTYROOM_COMMANDER, "[PK]", getName() + " has murdered " + target.getName())));
				}
			}
		}
	 ...
	 * @param killer the player who killed this instance
	 */
	public void calculatePvPAndPKInfo(L2PcInstance killer)
	{
		int kills = 0;
		
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(SELECT_CHARACTER_KILLS))
		{
			...
			{
				kills++;
				try (PreparedStatement statementUpdate = con.prepareStatement(UPDATE_CHARACTER_KILLS))
				{
					statementUpdate.setInt(1, kills);
					statementUpdate.setInt(2, getObjectId());
					statementUpdate.setInt(3, killer.getObjectId());
					statementUpdate.executeUpdate();
				}
				
				sendMessage("You have been killed " + kills + " times by " + killer.getName() + ".");
				killer.sendMessage("You have killed " + getName() + " " + kills + " times.");
			}
			else
			{
				try (PreparedStatement statementInsert = con.prepareStatement(INSERT_CHARACTER_KILLS))
				{
					statementInsert.setInt(1, getObjectId());
					statementInsert.setInt(2, killer.getObjectId());
					statementInsert.setInt(3, 1);
					statementInsert.executeUpdate();
				}
				
				sendMessage("This is the first time you have been killed by " + killer.getName() + ".");
				killer.sendMessage("You have killed " + getName() + " for the first time.");
			}
		}
		catch (Exception e)
		{
			LOGGER.error("L2PCInstance.calculatePvPAndPKInfo: Something went wrong while getting and updating data into character_kills table", e);
		}
	}
	...
	public void setPrivateStoreType(int type)
	{
		...
		if (privatestore == STORE_PRIVATE_NONE && getClient() == null)
		{
			this.store();
		}
	}
	...
	private boolean saveInDB()
	{
		...
		if (saved)
		{
			String text = "Created new character : " + getName() + " for account: " + accountName;
			L2Log.add(text, "characters_created", "characters_created");
		}
			...
			PreparedStatement statement = con.prepareStatement(SELECT_CHARACTER_BY_OBJ_ID);)
		{
			...
			try (ResultSet rset = statement.executeQuery())
			{
				while (rset.next())
				{
					...
					{
						LOGGER.error("L2PcIntance.restore: Possible invalid number format for the field 'base_class' for player with object id" + objectId + " in characters table", e);
					...
					{
						LOGGER.error("L2PcIntance.restore: Possible invalid hexadecimal format for the field 'name_color' and 'title_color' for player with object id" + objectId + " in characters table", e);
					}
			...
			if (player == null)
			{
				LOGGER.error("L2PcInstance.restore: Player is null, something went wrong!");
				return null;
			}
	...
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement(SELECT_CHARACTER_RECIPES))
		{
			statement.setInt(1, getObjectId());
			...
			try (ResultSet rset = statement.executeQuery())
			{
				while (rset.next())
				{
					Recipe recipe = RecipeTable.getInstance().getRecipeById(rset.getInt("id"));
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			if (effects != null && effects.length > 0)
			{
				try (PreparedStatement insertSkill = con.prepareStatement(INSERT_CHARACTER_SKILLS_SAVE))
				{
					...
					for (L2Effect effect : effects)
					{
						...
						else if (effect.getEffectType() == L2Effect.EffectType.COMBAT_POINT_HEAL_OVER_TIME)
						{
							continue;
						}
						
						else if (effect.getSkill().isToggle())
						{
							continue;
						}
						else if (effect.getStackType().equals("BattleForce"))
						{
							continue;
						}
						else if (effect.getStackType().equals("SpellForce"))
						{
							continue;
						}
						else if (effect.getSkill().getSkillType() == SkillType.FORCE_BUFF)
						{
							continue;
						}
						...
						if (effect.getInUse())
						{
							insertSkill.setInt(1, getObjectId());
							insertSkill.setInt(2, skillId);
							insertSkill.setInt(3, effect.getSkill().getLevel());
							insertSkill.setInt(4, effect.getCount());
							insertSkill.setInt(5, effect.getTime());
							insertSkill.setInt(6, ++buff_index);
			...
			try (PreparedStatement insertSkillReuseDelay = con.prepareStatement(INSERT_CHARACTER_SKILLS_REUSE_DELAY))
			{
				for (TimeStamp timeStamp : reuseTimeStamps.values())
				{
					...
					{
						insertSkillReuseDelay.setInt(1, getObjectId());
						insertSkillReuseDelay.setInt(2, timeStamp.getSkill().getId());
						insertSkillReuseDelay.setInt(3, timeStamp.getSkill().getLevel());
						insertSkillReuseDelay.setLong(4, timeStamp.getReuse());
						insertSkillReuseDelay.setLong(5, timeStamp.getStamp());
						insertSkillReuseDelay.setInt(6, getClassIndex());
						insertSkillReuseDelay.addBatch();
						counter++;
					}
				}
				...
				if (counter > 0)
				{
					insertSkillReuseDelay.executeBatch();
				}
			}
	...
	/**
	 * @return if player is in Seven Signs dungeon (Catacomb for example)
	 */
			...
			try (ResultSet rset = statement.executeQuery())
			{
				...
				while (rset.next())
				{
					...
					if (symbol_id != 0)
					{
						...
						if (tpl != null)
						{
							L2HennaInstance sym = new L2HennaInstance(tpl);
	...
	private void recalcHennaStats()
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			hennaMEN += playerHenna[i].getStatMEN();
	...
	public boolean isAutoAttackable(final L2Character attacker)
	{
		...
		else if (attacker instanceof L2PlayableInstance)
		{
			L2PcInstance player = null;
	...
	public void useMagic(final L2Skill skill, final boolean forceUse, final boolean dontMove)
	{
		...
		if (skill.isOffensive())
		{
			...
			if (!target.isAutoAttackable(this) && !forceUse && skill.getId() != 3261 && skill.getId() != 3260 && skill.getId() != 3262 && sklTargetType != SkillTargetType.TARGET_AURA && sklTargetType != SkillTargetType.TARGET_CLAN && sklTargetType != SkillTargetType.TARGET_ALLY && sklTargetType != SkillTargetType.TARGET_PARTY && sklTargetType != SkillTargetType.TARGET_SELF
	...
	public synchronized boolean setActiveClass(final int classIndex)
	{
		...
		stopAllEffects();
		// Call store() before modifying classIndex to avoid skill effects rollover. 2. Register the correct classId against applied 'classIndex'.
		store();
		...
		// 2. Remove all existing skills.
		// 3. Restore all the learned skills for the current class from the database.
		// 4. Restore effect/buff data for the new class.
		// 5. Restore henna data for the class, applying the new stat modifiers while removing existing ones.
		// 6. Reset HP/MP/CP stats and send Server->Client character status packet to reflect changes.
		// 7. Restore shortcut data related to this class.
		// 8. Resend a class change animation effect to broadcast to all nearby players.
		// 9.Unsummon any active servitor from the player.
	...
	public synchronized void deleteMe()
	{
		...
		DualBox.decreaseDualBoxCount(getIPAddress());
	...
	private void notifyFriends(final boolean closing)
	{
		...
		{
			final L2PcInstance friend = L2World.getInstance().getPlayerByName(friendName);
	...
	public void saveRecipeIntoDB(Recipe recipe)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement insertRecipe = con.prepareStatement(INSERT_CHARACTER_RECIPE))
		{
		...
		{
			LOGGER.error("L2PcInstance.saveRecipeIntoDB : Could not store recipe book data", e);
		}
	}
	
	public void setTelepotItemInformatio(int itemId, int itemAmount)
	{
		teleportItem.setItemId(itemId);
		teleportItem.setItemAmount(itemAmount);
	}
}