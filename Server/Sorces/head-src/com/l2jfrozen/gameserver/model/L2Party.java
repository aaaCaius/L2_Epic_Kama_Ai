...
public class L2Party
{
	...
	public synchronized void removePartyMember(final L2PcInstance player, final boolean sendMessage)
	{
		if (getPartyMembers().contains(player))
		{
			...
			if (isInCommandChannel())
			{
				...
				commandChannel.broadcastToChannelMembers(new ExOpenMPCC());
			}
	...
	/**
	 * @param name Player name who will become in new party leader
	 */
	public void changePartyLeader(String name)
	{
		L2PcInstance newPartyLeader = getPlayerByName(name);
		
		if (newPartyLeader != null && !newPartyLeader.isInDuel())
		{
			if (getPartyMembers().contains(newPartyLeader))
			{
				if (isLeader(newPartyLeader))
				{
					newPartyLeader.sendPacket(new SystemMessage(SystemMessageId.YOU_CANNOT_TRANSFER_RIGHTS_TO_YOURSELF));
				}
				else
				{
					// Swap party members
					L2PcInstance oldPartyLeader = getLeader();
					int partyPosition = getPartyMembers().indexOf(newPartyLeader);
					getPartyMembers().set(0, getPartyMembers().get(partyPosition));
					getPartyMembers().set(partyPosition, oldPartyLeader);
					...
					if (isInCommandChannel())
					{
						if (commandChannel.getChannelLeader() == oldPartyLeader)
						{
							commandChannel.setChannelLeader(getLeader());
					...
					if (newPartyLeader.isInPartyMatchRoom())
					{
						PartyMatchRoom room = PartyMatchRoomList.getInstance().getPlayerRoom(newPartyLeader);
						room.changeLeader(newPartyLeader);
					}
				}
			}
			else
			{
				newPartyLeader.sendPacket(new SystemMessage(SystemMessageId.YOU_CAN_TRANSFER_RIGHTS_ONLY_TO_ANOTHER_PARTY_MEMBER));
			}
		}
	}
	 ...
	 * @param target
	 */
	public void distributeItem(final L2PcInstance player, final RewardHolder item, final boolean spoil, final L2Attackable target)
	{
		if (item == null)
		{
			return;
		...
		if (item.getRewardId() == 57)
		{
			distributeAdena(player, item.getRewardCount(), target);
			return;
		}
		
		L2PcInstance looter = getActualLooter(player, item.getRewardId(), spoil, target);
		
		looter.addItem(spoil ? "Sweep" : "Party", item.getRewardId(), item.getRewardCount(), player, true);
		...
		if (item.getRewardCount() > 1)
		{
			...
			msg.addItemName(item.getRewardId());
			msg.addNumber(item.getRewardCount());
	...
	public void distributeAdena(final L2PcInstance player, final int adena, final L2Character target)
	{
		...
		List<L2PcInstance> rewards = new ArrayList<>();
		...
		final int count = adena / rewards.size();
		
		for (L2PcInstance member : rewards)
		{
