...
public class Castle
{
	...
	private static final String SELECT_CASTLE_BY_ID = "SELECT id, `name`, taxPercent, treasury, siegeDate, siegeDayOfWeek, siegeHourOfDay FROM castle WHERE id=?";
	private static final String SELECT_CLAN_ID_FROM_CLAN_DATA = "SELECT clan_id FROM clan_data WHERE hasCastle=?";
	...
	private void load()
	{
		try(Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			try(PreparedStatement statement = con.prepareStatement(SELECT_CASTLE_BY_ID))
			{
			...
			try(PreparedStatement statement = con.prepareStatement(SELECT_CLAN_ID_FROM_CLAN_DATA))
			{
				statement.setInt(1, getCastleId());
		...
		{
			LOGGER.error("Castle.load: Something went wrong while loading castles", e);
		}
	}
	...
	public void updateCrop(int cropId, int amount, int period)
	{
		...
		{
			LOGGER.info("Castle.updateCrop: Error adding crop data for castle " + getName(), e);
		}
	}
