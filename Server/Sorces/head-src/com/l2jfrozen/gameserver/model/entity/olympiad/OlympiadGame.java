...
class OlympiadGame
{
	...
	protected OlympiadGame(final int id, final COMP_TYPE type, final L2FastList<L2PcInstance> list)
	{
		...
		if (list != null)
		{
			...
			if (Config.ENABLE_OLYMPIAD_DEBUG)
			{
				...
				L2Log.add(text, "Olympiad_game_logs");
			}
		}
		else
		{
	...
	protected void removals()
	{
		...
		for (final L2PcInstance player : players)
		{
			try
			{
				...
				if (Config.REMOVE_SUMMON_AND_CUBIC_OLYMPIAD)
				{
					...
					if(summon != null)
					{
						// Do not unsummon Soultaker summons (Corrupted man, Reanimated man or Cursed man) cause you need corpses in order to summon them
						if(player.getClassId() != ClassId.Soultaker)
						{
	...
	protected void validateWinner()
	{
		...
		StatsSet playerOneStat = Olympiad.getInstance().getNobleStats(playerOne.getObjectId());
		StatsSet playerTwoStat = Olympiad.getInstance().getNobleStats(playerTwo.getObjectId());
		...
		if (playerOneDefaulted || playerTwoDefaulted)
		{
			if (playerOneDefaulted)
			{
				...
				Olympiad.getInstance().updateNobleStats(playerOne.getObjectId(), playerOneStat);
			...
			if (playerTwoDefaulted)
			{
				...
				Olympiad.getInstance().updateNobleStats(playerTwo.getObjectId(), playerTwoStat);
	...
	protected void announceGame()
	{
		for (L2Spawn manager : Olympiad.olymanagers)
		{
			if (manager != null && manager.getLastSpawn() != null)
			{
				...
				CreatureSay cs = new CreatureSay(objId, 1, npcName, "Olympiad is going to begin in Arena " + (stadiumID + 1) + " in a moment.");
