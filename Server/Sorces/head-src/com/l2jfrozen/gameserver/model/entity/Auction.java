...
public class Auction
{
	...
	private void updateInDB(final L2PcInstance bidder, final int bid)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();)
		{
			...
			else
			{
				...
				if (L2World.getInstance().getPlayerByName(highestBidderName) != null)
				{
					L2World.getInstance().getPlayerByName(highestBidderName).sendMessage("You have been out bidded");
				}
			}
	...
	private void removeBids()
	{
		...
		for (final Bidder b : bidders.values())
		{
			...
			{
				if (L2World.getInstance().getPlayerByName(b.getName()) != null)
				{
					L2World.getInstance().getPlayerByName(b.getName()).sendMessage("Congratulation you have won ClanHall!");
				}
			}
