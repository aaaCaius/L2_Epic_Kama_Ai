...
public class SevenSigns
{
	...
	private static final String INSERT_7_SIGNS_PLAYER_INFO = "INSERT INTO character_seven_signs (char_obj_id, cabal, seal) VALUES (?,?,?)";
	private static final String SELECT_7_SIGNS_CONTRIBUTION = "SELECT char_obj_id, cabal, seal, red_stones, green_stones, blue_stones, ancient_adena_amount, contribution_score FROM character_seven_signs";
	...
	private static final String UPDATE_7_SIGNS_STATUS_DATE_BY_ID_0 = "UPDATE seven_signs_status SET date=? WHERE id=0";
	private static final String UPDATE_7_SIGNS_CONTRIBUTION_BY_CHAR_OBJ_ID = "UPDATE character_seven_signs SET cabal=?, seal=?, red_stones=?, green_stones=?, blue_stones=?, ancient_adena_amount=?, contribution_score=? WHERE char_obj_id=?";
	...
	public static final int PERIOD_START_MINUTE = 00;
	...
	private final Map<Integer, Integer> signsSealOwners = new HashMap<>();
	private final Map<Integer, Integer> signsDuskSealTotals = new HashMap<>();
	private final Map<Integer, Integer> signsDawnSealTotals = new HashMap<>();
	...
	public void spawnSevenSignsNPC()
	{
		...
		if (isSealValidationPeriod() || isCompResultsPeriod())
		{
			...
			{
				if (!Config.ALT_ANNOUNCE_MAMMON_SPAWN)
				{
		...
		switch (getCurrentPeriod())
		{
			...
			case PERIOD_COMPETITION:
				...
				if (daysToChange == 7)
				{
					...
					else if (calendar.get(Calendar.HOUR_OF_DAY) == PERIOD_START_HOUR && calendar.get(Calendar.MINUTE) < PERIOD_START_MINUTE)
					{
						daysToChange = 0;
					}
				}
				...
				calendar.set(Calendar.MINUTE, PERIOD_START_MINUTE);
	...
	protected void restoreSevenSignsData()
	{
		try(Connection con = L2DatabaseFactory.getInstance().getConnection();)
		{
			...
			try(PreparedStatement statement = con.prepareStatement(SELECT_7_SIGNS_STATUS_BY_ID_0);
				ResultSet rset = statement.executeQuery())
			{
				while (rset.next())
				{
			...
			try(PreparedStatement statement = con.prepareStatement(UPDATE_7_SIGNS_STATUS_DATE_BY_ID_0))
			{
	...
	public void saveSevenSignsData(L2PcInstance player, boolean updateSettings)
	{
		...
		try(Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			if(player != null)
			{
				try(PreparedStatement statement = con.prepareStatement(UPDATE_7_SIGNS_CONTRIBUTION_BY_CHAR_OBJ_ID))
				{
					StatsSet sevenSignsContribution = signsPlayerData.get(player.getObjectId());
					
					if(sevenSignsContribution != null)
					{
						statement.setString(1, sevenSignsContribution.getString("cabal"));
						statement.setInt(2, sevenSignsContribution.getInteger("seal"));
						statement.setInt(3, sevenSignsContribution.getInteger("red_stones"));
						statement.setInt(4, sevenSignsContribution.getInteger("green_stones"));
						statement.setInt(5, sevenSignsContribution.getInteger("blue_stones"));
						statement.setDouble(6, sevenSignsContribution.getDouble("ancient_adena_amount"));
						statement.setDouble(7, sevenSignsContribution.getDouble("contribution_score"));
						statement.setInt(8, sevenSignsContribution.getInteger("char_obj_id"));
						statement.executeUpdate();
						
						if (Config.DEBUG)
						{
							LOGGER.info("SevenSigns: Updated data in database for char ID " + sevenSignsContribution.getInteger("char_obj_id") + " (" + sevenSignsContribution.getString("cabal") + ")");
						}
					}
				}
			}
		...
		{
			LOGGER.error("SevenSigns.saveSevenSignsData: Unable to save data to database", e);
		}
	}
	...
	protected void calcNewSealOwners()
	{
		...
		for (final Integer currSeal : signsDawnSealTotals.keySet())
		{
			 ...
			 * If they chose less than 35%, they cannot own the Seal. - If the Seal was owned by the winner in the previous Seven Signs, they can retain that seal if 10% or more members have chosen it. 
