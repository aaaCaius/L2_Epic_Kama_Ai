...
public class L2Attackable extends L2NpcInstance
{
	...
	private RewardHolder[] sweepItems;
	
	/** crops */
	private RewardHolder[] harvestItems;
	...
	public L2Character getMostHated()
	{
		...
		synchronized (getAggroList())
		{
			...
			for (final AggroInfoHolder ai : getAggroListRP().values())
			{
				...
				if (ai.getAttacker().isAlikeDead() || !getKnownList().knowsObject(ai.getAttacker()) || !ai.getAttacker().isVisible() || ai.getAttacker() instanceof L2PcInstance && !((L2PcInstance) ai.getAttacker()).isOnline()) // if player go
																																																									// in offline
																																																									// mode, he
																																																									// must be hated
																																																									// however
	 ...
	 * @return
	 */
	private RewardHolder calculateRewardItem(final L2PcInstance lastAttacker, final L2DropData drop, final int levelModifier, final boolean isSweep)
	{
	 ...
	 * @return
	 */
	private RewardHolder calculateCategorizedRewardItem(final L2PcInstance lastAttacker, final L2DropCategory categoryDrops, final int levelModifier)
	{
	...
	public void doItemDrop(final L2NpcTemplate npcTemplate, final L2Character lastAttacker)
	{
		...
		for (final L2DropCategory cat : npcTemplate.getDropData())
		{
			...
			if (cat.isSweep())
			{
				...
				if (isSpoil()/* && !isSeeded() */)
				{
					List<RewardHolder> sweepList = new ArrayList<>();
					...
					for (final L2DropData drop : cat.getAllDrops())
					{
						...
						if (Config.DEBUG)
						{
							LOGGER.debug("Item id to spoil: " + item.getRewardId() + " amount: " + item.getRewardCount());
						}
					...
					{
						sweepItems = sweepList.toArray(new RewardHolder[sweepList.size()]);
					}
			...
			else
			{
				...
				if (item != null)
				{
					if (Config.DEBUG)
					{
						LOGGER.debug("Item id to drop: " + item.getRewardId() + " amount: " + item.getRewardCount());
					}
					...
					if (Config.AUTO_LOOT)
					{
						...
						if (item_templ == null)
						{
							LOGGER.info("ERROR: Item id to autoloot " + item.getRewardId() + " has not template into items/armor/weapon tables.. It cannot be dropped..");
					...
					if (this instanceof L2RaidBossInstance || this instanceof L2GrandBossInstance)
					{
						...
						sm.addItemName(item.getRewardId());
						sm.addNumber(item.getRewardCount());
						broadcastPacket(sm);
						sm = null;
					}
				}
		...
		if (rateHp < 2 && String.valueOf(npcTemplate.type).contentEquals("L2Monster")) // only L2Monster with <= 1x HP can drop herbs
		{
			...
			{
				RewardHolder item = new RewardHolder(8612, 1); // Herb of Warrior
				...
				if (Config.AUTO_LOOT && Config.AUTO_LOOT_HERBS)
				{
					...
					{
						player.addItem("Loot", item.getRewardId(), item.getRewardCount(), this, true);
					}
				}
				else
				{
			...
			else
			{
				for (int i = 0; i < 3; i++)
				{
					...
					if (random < Config.RATE_DROP_COMMON_HERBS)
					{
						...
						if (i == 0)
						{
							item = new RewardHolder(8606, 1); // Herb of Power
						}
						if (i == 1)
						{
							item = new RewardHolder(8608, 1); // Herb of Atk. Spd.
						}
						if (i == 2)
						{
							item = new RewardHolder(8610, 1); // Herb of Critical Attack
						}
			...
			{
				RewardHolder item = new RewardHolder(8613, 1); // Herb of Mystic
			...
			else
			{
				for (int i = 0; i < 2; i++)
				{
					...
					if (random < Config.RATE_DROP_COMMON_HERBS)
					{
						...
						if (i == 0)
						{
							item = new RewardHolder(8607, 1); // Herb of Magic
						}
						if (i == 1)
						{
							item = new RewardHolder(8609, 1); // Herb of Casting Speed
						}
						if (item == null)
						{
							break;
			...
			if (random < Config.RATE_DROP_SPECIAL_HERBS && !spec)
			{
				RewardHolder item = new RewardHolder(8614, 1); // Herb of Recovery
			...
			if (!hp)
			{
				...
				{
					RewardHolder item = new RewardHolder(8600, 1); // Herb of Life
			...
			if (!hp)
			{
				...
				{
					RewardHolder item = new RewardHolder(8601, 1); // Greater Herb of Life
			...
			if (!hp)
			{
				...
				{
					RewardHolder item = new RewardHolder(8602, 1); // Superior Herb of Life
			...
			if (!mp)
			{
				...
				if (random < Config.RATE_DROP_MP_HP_HERBS)
				{
					RewardHolder item = new RewardHolder(8603, 1); // Herb of Mana
			...
			if (!mp)
			{
				...
				if (random < Config.RATE_DROP_GREATER_HERBS)
				{
					RewardHolder item = new RewardHolder(8604, 1); // Greater Herb of Mana
			...
			if (!mp)
			{
				...
				if (random < Config.RATE_DROP_SUPERIOR_HERBS)
				{
					RewardHolder item = new RewardHolder(8605, 1); // Superior Herb of Mana
			...
			if (random < Config.RATE_DROP_COMMON_HERBS)
			{
				RewardHolder item = new RewardHolder(8611, 1); // Herb of Speed
	...
	public L2ItemInstance DropItem(final L2PcInstance mainDamageDealer, final RewardHolder item)
	{
		...
		for (int i = 0; i < item.getRewardCount(); i++)
		{
			...
			ditem = ItemTable.getInstance().createItem("Loot", item.getRewardId(), item.getRewardCount(), mainDamageDealer, this);
			ditem.getDropProtection().protect(mainDamageDealer);
			ditem.dropMe(this, newX, newY, newZ);
			
			// Add drop to auto destroy item task
			if (!Config.LIST_PROTECTED_ITEMS.contains(item.getRewardId()))
			{
	 ...
	 * @return
	 */
	public synchronized RewardHolder[] takeSweep()
	{
		final RewardHolder[] sweep = sweepItems;
	...
	public void setSweepItems(RewardHolder[] sweep)
	{
		sweepItems = sweep;
	}
	 ...
	 * @return
	 */
	public synchronized RewardHolder[] takeHarvest()
	{
		final RewardHolder[] harvest = harvestItems;
	...
	public void setSeeded(final int id, final int seederLvl)
	{
		...
		List<RewardHolder> harvested = new ArrayList<>();
		
		harvested.add(new RewardHolder(L2Manor.getInstance().getCropType(seedType), count * Config.RATE_DROP_MANOR));
		
		harvestItems = harvested.toArray(new RewardHolder[harvested.size()]);
