...
public class Quest extends ManagedScript
{
	...
	private static final String SELECT_CHARACTER_QUEST_BY_CHAR_ID = "SELECT name,var,value FROM character_quests WHERE char_id=?";
	private static final String SELECT_CHARACTER_QUEST_BY_CHAR_ID_AND_VAR = "SELECT name,value FROM character_quests WHERE char_id=? AND var=?";
	private static final String DELETE_CHARACTER_QUEST_INVALID_STATE = "DELETE FROM character_quests WHERE char_id=? AND name=?";
	private static final String DELETE_CHARACTER_QUEST_INVALID_DATA = "DELETE FROM character_quests WHERE char_id=? AND name=? AND var=?";
	...
	public boolean showError(final L2Character object, final Throwable t)
	{
		...
		{
			LOGGER.error(getScriptFile().getAbsolutePath(), t);
		}
	...
	public static void onPlayerEnter(L2PcInstance player)
	{
		if (Config.DEBUG)
		{
			LOGGER.info("Quest.onPlayerEnter " + player.getName());
		...
		{
			try(PreparedStatement statement = con.prepareStatement(SELECT_CHARACTER_QUEST_BY_CHAR_ID_AND_VAR))
			{
				...
				try(ResultSet rs = statement.executeQuery())
				{
					while (rs.next())
					{
						...
						if (quest == null)
						{
							...
							{
								try(PreparedStatement invalidQuestData = con.prepareStatement(DELETE_CHARACTER_QUEST_INVALID_STATE))
								{
									invalidQuestData.setInt(1, player.getObjectId());
									invalidQuestData.setString(2, questName);
									invalidQuestData.executeUpdate();
									
									LOGGER.warn("Quest.onPlayerEnter: Invalid quest state " + stateId + " deleted  in quest " + questName + " for player " + player.getName());
								}
							}
						...
						if (stateId.equalsIgnoreCase("Completed"))
						{
							completed = true;
						}
						
						// Create an object State containing the state of the quest
						State state = quest.states.get(stateId);
						
						if (state == null)
						{
							if (Config.DEVELOPER)
							{
								LOGGER.info("Unknown state in quest " + questName + " for player " + player.getName());
							}
						...
						QuestState questState = new QuestState(quest, player, state, completed);
						player.setQuestState(questState);
					}
				}
			}
			
			try(PreparedStatement statement = con.prepareStatement(SELECT_CHARACTER_QUEST_BY_CHAR_ID))
			{
				statement.setInt(1, player.getObjectId());
				...
				try(ResultSet rs = statement.executeQuery())
				{
					while (rs.next())
					{
						...
						QuestState questState = player.getQuestState(questName);
						
						if (questState == null)
						{
							if (Config.AUTODELETE_INVALID_QUEST_DATA)
							{
								try(PreparedStatement invalidQuestDataVar = con.prepareStatement(DELETE_CHARACTER_QUEST_INVALID_DATA))
								{
									...
									LOGGER.warn("Quest.onPlayerEnter: Invalid variable " + var + " deleted in quest " + questName + " for player " + player.getName());
								}
							}
						...
						questState.setInternal(var, value);
					}
				}
			}
		}
		...
		{
			LOGGER.error("Could not select character quest", e);
		}
