	...
	public void useSkill(final L2Character activeChar, final L2Skill skill, final L2Object[] targets)
	{
		...
		for (final L2Object aTargetList : targetList)
		{
			...
			if (targetHarvest.isSeeded())
			{
				if (calcSuccess())
				{
					RewardHolder[] items = targetHarvest.takeHarvest();
					if (items != null && items.length > 0)
					{
						for (final RewardHolder ritem : items)
						{
							cropId = ritem.getRewardId(); // always got 1 type of crop as reward
							...
							{
								L2ItemInstance item = activeCharHarvest.getInventory().addItem("Manor", ritem.getRewardId(), ritem.getRewardCount(), activeCharHarvest, targetHarvest);
								if (iu != null)
								{
									iu.addItem(item);
								}
								send = true;
								total += ritem.getRewardCount();
								item = null;
							}
						}
						if (send)
