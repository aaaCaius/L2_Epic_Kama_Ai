...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(final String command, final L2PcInstance activeChar)
	{
		...
		switch (comm)
		{
			...
			case admin_character_info:
			{
				...
				final L2PcInstance target = L2World.getInstance().getPlayerByName(val);
	...
	{
		List<L2PcInstance> onlinePlayers = L2World.getInstance().getAllPlayers().stream().filter(player -> player != null && player.isOnline()).collect(Collectors.toList());
	...
	private void findCharactersPerAccount(final L2PcInstance activeChar, final String characterName) throws IllegalArgumentException
	{
		if (characterName.matches(Config.CNAME_TEMPLATE))
		{
			...
			L2PcInstance player = L2World.getInstance().getPlayerByName(characterName);
