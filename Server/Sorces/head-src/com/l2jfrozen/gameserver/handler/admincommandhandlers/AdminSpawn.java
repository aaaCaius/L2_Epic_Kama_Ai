...
public class AdminSpawn implements IAdminCommandHandler
{
	...
	private void spawnMonster(L2PcInstance activeChar, String monsterId, int respawnTime, int mobCount, boolean permanent)
	{
		...
		if (monsterId.matches("[0-9]*"))
		{
			...
			npcTemplate = NpcTable.getInstance().getTemplate(monsterTemplate);
		}
		else
		{
			// First parameter wasn't just numbers so go by name not ID
			monsterId = monsterId.replace('_', ' ');
			npcTemplate = NpcTable.getInstance().getTemplateByName(monsterId);
		}
		...
		try
		{
			...
			{
				activeChar.sendMessage("Another instance of " + npcTemplate.name + " already present into database:");
			...
			activeChar.sendMessage("Created " + npcTemplate.name + " on " + target.getObjectId());
		}
		catch (Exception e)
		{
